// this is for showing full post by using their postid
...
export default async function PostIdRoute({ params} : PostIdRouteProps ){
  ...
  //this is for optimization, here query run parallely they not depend on each other
  const [post, preLoadedComments, userId] = await Promise.all([
    await fetchQuery(api.posts.getPostById, {postId: postId}),

      //here we load comment then pass to commentssection by props no loading state
    await preloadQuery(api.comments.getCommentByPostId, {postId: postId}),

    //for check user id of currently logged in user to track presence
